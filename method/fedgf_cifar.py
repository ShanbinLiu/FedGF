...
class Server(BaseServer):
    ...
    def run(self):
        ...
        for round in range(self.num_rounds):
            ...
            if self.eval_interval>0 and (round==0 or round%self.eval_interval==0):
                ...
                gini_val=self.gini(accs_vaild)
                ave_vaild_accuracy.append(np.mean(accs_vaild))
                gini_vaild_list.append(gini_val)
    ...
    def iterate(self, t):
        ...
        if len(self.client_weights)==0:
            list_select = [self.clients[i].datasize for i in selected_clients]
            num = sum(list_select)
            self.client_weights=[1.0*self.client_vols[id]/num for id in selected_clients]
        # training
        for cid in selected_clients:
            vaild_acc, _ = modelfuncs.test(self.model, self.clients[cid].val_data, self.clients[cid].ldr_vaild, self.batchsize)
            vailds_acc.append(vaild_acc/100+1e-10)
        start_time=time.time()
        gini_vaild = self.gini(vailds_acc)
        self.gini_evaluation_list.append(gini_vaild)
        if t >= self.threshold and t % self.threshold_half == 0 and np.mean(self.gini_evaluation_list[t - self.threshold:t-self.threshold_half]) - np.mean(self.gini_evaluation_list[t - self.threshold_half:t]) < self.eta:
            self.flag = True
        end_time = time.time()
        ...
        if self.flag:
            falseweight = []
            for client_idx in range(len(selected_clients)):
                falseweight.append(1 - vailds_acc[client_idx])
            print(falseweight)
            ...
            total = f_exp.sum()
            for client_idxx in range(len(selected_clients)):
