...
def read_option():
    ...
    # basic settings
    parser.add_argument('--method', help='name of method;', type=str, default='feddog1')
    parser.add_argument('--dataset', help='name of dataset;', type=str, default='cifar10')
    parser.add_argument('--model', help='name of model;', type=str, default='mlp')
    ...
    parser.add_argument('--weight_decay', help='momentum of local update', type=float, default=0)
    # controlling
    ...
    parser.add_argument('--temperature', help='temperature in feddog', type=float, default='0.1')
    parser.add_argument('--lmd', help='lambda in feddog', type=float, default='0.1')
    parser.add_argument('--lamda', help='lambda in fedgi', type=float, default='0.1')
    parser.add_argument('--epsilons', help='epsilons in FedGini', type=float, default='0.5')
    parser.add_argument('--window', help='window in FedGini', type=int, default='150')
    parser.add_argument('--k', help='k in fedla', type=float, default='1.0')
    parser.add_argument('--threshold', help='threshold in fedgi', type=int, default='1')
    parser.add_argument('--eta', help='eta in FedGF', type=float, default='0.1')
...
def initialize(option):
    ...
    vaild_path = os.path.join('task', option['dataset'], 'data', 'vaild')
    ...
    vaild_files = os.listdir(vaild_path)
    vaild_files = [f for f in vaild_files if f.endswith('.json')]
    for f in vaild_files:
        file_path = os.path.join(vaild_path, f)
        with open(file_path, 'r') as inf:
            cdata = json.load(inf)
        vaild_data.update(cdata['user_data'])
    ...
    for cname in client_names:
        clients.append(Client(option=option, name=cname, data_train_dict=train_data[cname], data_test_dict=test_data[cname], partition = True))
    print('ok')
    # init server
